...
from tkinter import filedialog, messagebox, Frame, Label, Button, Entry
import pandas as pd
...
model = load(r'C:\Users\manve\Downloads\Research AERO\Partner Matching Algorithm\ML\final_model.joblib')
...
def format_meeting_times_commas_corrected_fixed(row, time_slot_columns):
    ...
    for col in time_slot_columns:
        if not pd.isna(row[col]):
            ...
            # Check if hour is a valid digit and convert to integer, if not skip this column
            if hour.isdigit():
                hour = int(hour)
            else:
                continue
    ...
    meeting_times_sorted = sort_meeting_times(meeting_times)
    return ", ".join(meeting_times_sorted)
...
def prepare_data_final(df, student_type, priority_options, non_priority_options):
    ...
    excluded_volunteers = pd.DataFrame()
    #if student_type == 'domestic':
        #feedback_col = "Please indicate your first and second choices. [Microteaching Feedback Sessions ]"
        #if feedback_col in df.columns:
            #first_choice_volunteers = df[df[feedback_col] == 'First Choice'].nsmallest(0, 'Timestamp')
            #excluded_volunteers = first_choice_volunteers.copy()
            # Exclude these volunteers from the cleaned_df
            #cleaned_df = cleaned_df[~cleaned_df['UIN'].isin(first_choice_volunteers['UIN'])]
    ...
    return cleaned_df, excluded_volunteers
...
def process_files(ns_entry, nns_entry):
    try:
        ns_file_path = ns_entry.get()
        nns_file_path = nns_entry.get()
        if ns_file_path and nns_file_path:
            ...
            # Process the domestic and international files
            domestic_cleaned_df_final, excluded_volunteers = prepare_data_final(domestic_df_new, 'domestic',
                                                                                priority_options, non_priority_options)
            international_cleaned_df_final, _ = prepare_data_final(international_df_new, 'international',
                                                                   priority_options, non_priority_options)
            ...
            with pd.ExcelWriter(output_stream) as writer:
                ...
                if not excluded_volunteers.empty:
                    excluded_volunteers.to_excel(writer, sheet_name='Excluded Volunteers', index=False)
            output_stream.seek(0)

            # Let the user choose where to save the Excel file
...
# GUI setup
root = tk.Tk()
root.title("CSV Match Processor")
root.attributes('-fullscreen', True)  # Make the application fullscreen

# TAMU Maroon color background
root.configure(bg='#500000')

# Create a frame for the form
form_frame = Frame(root, bg='white', padx=10, pady=10)
form_frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

# Title label
title_label = Label(form_frame, text="Upload Data for Matching", font=("Helvetica", 18, "bold"), bg='white')
title_label.pack(pady=(0, 20))

# NS File Entry
ns_label = Label(form_frame, text="English-Dominant Speakers (NS) File:", bg='white')
ns_label.pack()
ns_entry = Entry(form_frame, width=50)
ns_entry.pack(pady=(0, 10))
ns_button = Button(form_frame, text="Choose File", command=lambda: choose_file(ns_entry))
ns_button.pack()

# NNS File Entry
nns_label = Label(form_frame, text="Non-English-Dominant Speakers (NNS) File:", bg='white')
nns_label.pack()
nns_entry = Entry(form_frame, width=50)
nns_entry.pack(pady=(0, 10))
nns_button = Button(form_frame, text="Choose File", command=lambda: choose_file(nns_entry))
nns_button.pack()

# Process Button
process_button = Button(form_frame, text="Process Matching", command=lambda: process_files(ns_entry, nns_entry))
process_button.pack(pady=20)

# Quit Button - to exit fullscreen mode
quit_button = Button(root, text="Exit Fullscreen", command=root.quit, bg='white')
quit_button.place(relx=0.5, rely=0.95, anchor=tk.CENTER)
